import {updateFlightCues} from './flight-cues.js';
...
import {viewFromViewer,parseViewHash,viewURL,readSession,writeSession,restoreView} from './view-state.js';
let leave=()=>{};
function enter(){
 ...
 const compactNotes=matchMedia('(max-width:960px)'),notebook=el('notebook');
 const sizeNotebook=()=>{notebook.open=!compactNotes.matches;};
 sizeNotebook();compactNotes.addEventListener('change',sizeNotebook,{signal});
 ...
 const saved=readSession(storage),linked=parseViewHash(location.hash);
 mission=linked?.mission||saved.active;let pending=linked||saved.views[mission];
 if(pending){stage=missions[mission].stages.findIndex(s=>s.id===pending.chapter);progress=pending.progress;}
 function remember(){
  const state=ready&&viewer?.state.mission===mission?viewFromViewer(viewer.state):pending||viewFromViewer({mission,stage,progress});
  if(state)saved.views[mission]=state;saved.active=mission;writeSession(storage,saved);
 }
 function clearShare(){shareVersion++;el('share-manual').hidden=true;el('share-status').textContent='';action('copy').disabled=!ready;}
 ...
 function touchMode(active){
  ...
  el('viewport').toggleAttribute('data-touch-active',active);viewer?.touch(active);el('hint').textContent=coarse.matches?(active?'Drag to rotate · pinch to zoom · inspection pauses playback':'Tap Explore to rotate · inspection pauses playback'):'Drag to orbit · Ctrl/⌘ + scroll to zoom · inspection pauses playback';
 }
 function busy(value,message='Preparing the mission model…'){
  if(value){el('range').textContent='Preparing trajectory…';el('utc').textContent='';}
 ...
 function chapterUI(){
  ...
  el('scene').textContent=scales[s.kind];if(!ready){el('range').textContent=root.dataset.ready==='error'?'Trajectory view unavailable':'Preparing trajectory…';el('utc').textContent='';}
  ...
  updateStory(root,m,stage);
  el('chapters').querySelectorAll('[data-mission-stage]').forEach(b=>{if(Number(b.dataset.missionStage)===stage)b.setAttribute('aria-current','step');else b.removeAttribute('aria-current');});
 ...
 function controlsUI(){
  ...
  updateFlightCues(root,state,kind,mission,missions[mission].target);
 ...
 function select(index,p=0){
  clearShare();
  pending=null;stage=Math.max(0,Math.min(missions[mission].stages.length-1,index));progress=clampProgress(p);playing=false;if(ready){viewer.play(false);viewer.select(stage,progress);}chapterUI();controlsUI();animateChapter();remember();
 }
 function setProgress(p){if(!ready)return;clearShare();progress=clampProgress(p);playing=false;viewer.progress(progress);controlsUI();remember();}
 function error(){playing=false;busy(true,'The 3D view could not load. Explore the chapters and mission sources below.');controlsUI();root.dataset.ready='error';el('range').textContent='Trajectory view unavailable';action('retry').hidden=false;}
 async function start(){
  ...
  try{
   ...
   viewer=createViewer(el('viewport'),{signal,onReady:()=>{if(disposed)return;const restore=pending||viewFromViewer({mission,stage,progress});busy(false);action('retry').hidden=true;restoreView(viewer,restore);pending=null;playing=false;chapterUI();controlsUI();remember();viewer.visible(visible&&!document.hidden);},
    onError:error,onTick:state=>{if(disposed||!ready||state.mission!==mission)return;const changed=stage!==state.stage;stage=state.stage;progress=state.progress;playing=state.playing;if(changed){chapterUI();animateChapter();}controlsUI();}});
 ...
 async function choose(id,view){
  if(!Object.hasOwn(missions,id)||id===mission)return;clearShare();remember();mission=id;pending=view||saved.views[id];stage=pending?missions[id].stages.findIndex(s=>s.id===pending.chapter):0;progress=pending?.progress||0;playing=false;
  el('share-manual').hidden=true;el('share-status').textContent='';
  missionUI(id);busy(true);chapterUI();controlsUI();if(viewer)await viewer.load(id);else await start();
 }
 async function copyView(){
  const url=viewURL(location.href,viewFromViewer(viewer?.state));if(!url)return;
  const ticket=++shareVersion;action('copy').disabled=true;el('share-manual').hidden=true;
  try{await navigator.clipboard.writeText(url);if(!disposed&&ticket===shareVersion)el('share-status').textContent='View link copied. It opens paused.';}
  catch{if(!disposed&&ticket===shareVersion){el('share-status').textContent='Copy unavailable. Select and copy the link below.';el('share-url').value=url;el('share-manual').hidden=false;el('share-url').focus();el('share-url').select();}}
  finally{if(!disposed&&ticket===shareVersion)action('copy').disabled=!ready;}
 }
 function restoreHash(){
  const view=parseViewHash(location.hash);if(!view)return;clearShare();
  if(view.mission!==mission){choose(view.mission,view);return;}
  pending=view;stage=missions[mission].stages.findIndex(s=>s.id===view.chapter);progress=view.progress;playing=false;
  if(ready){restoreView(viewer,view);pending=null;}chapterUI();controlsUI();remember();
 }
 ...
 root.addEventListener('click',e=>{
  ...
  else if(a){viewer?.action(a);controlsUI();}
  if(a)remember();
 },{signal});
 ...
 document.addEventListener('visibilitychange',()=>{if(document.hidden)remember();viewer?.visible(visible&&!document.hidden);},{signal});
 ...
 if(linked&&history.state?.madsScrollY===undefined)requestAnimationFrame(()=>{if(!disposed){root.scrollIntoView({block:'start',behavior:'instant'});start();}});
 leave=()=>{shareVersion++;remember();disposed=true;document.documentElement.removeAttribute('data-mission-in-view');abort.abort();observer.disconnect();displayObserver.disconnect();viewer?.dispose();root.querySelector('.mission-copy').getAnimations().forEach(a=>a.cancel());delete root.dataset.initialized;if(window.sampleMissions===api)delete window.sampleMissions;};
}
