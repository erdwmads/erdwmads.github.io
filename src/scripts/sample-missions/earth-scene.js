...
function releaseFrame(state){
 ...
 const wing=outward.clone().cross(new T.Vector3(...state.forward)).normalize();
...
export function earthFlightShot(state,context,aspect=1.9){
 const portrait=Math.max(1,Math.min(1.65,1.18/aspect)),position=new T.Vector3(...state.positionKm);let target,offset,size;
 if(context==='earth'){
  ...
  if(state.kind==='return'){
   const radial=new T.Vector3(...state.up),east=new T.Vector3(radial.y,-radial.x,0),frame=releaseFrame(state);
   // Keep Earth behind the initial close view, then move sideways as the two
   // trajectories become aligned with the local radial viewing direction.
   const side=.35+1.2*Math.abs(radial.dot(frame.outward))**6;
   offset=east.multiplyScalar(.3).addScaledVector(radial,2.4).addScaledVector(frame.wing,side).multiplyScalar(size*portrait);
  }
  else {
   const radial=new T.Vector3(...state.up),east=new T.Vector3(radial.y,-radial.x,0),south=east.clone().cross(radial);
   offset=east.multiplyScalar(1.25).addScaledVector(radial,state.kind==='launch'?.5:.85).addScaledVector(south,2.15).multiplyScalar(size*portrait);
  }
 }
 return {target:target.toArray(),position:target.clone().add(offset).toArray(),fov:40,near:Math.max(.0000005,size*.005),far:Math.max(40000,position.distanceTo(center)*4),minDistance:size*.8,maxDistance:Math.max(50000,size*8)};
}
export function createEarthFlightScene(id,{group,earth,craft,launch,capsule,canopy,desert,heat,entryWake}){
 ...
 function hide(){smoke.visible=false;local.visible=false;helpers(false);state=null;}
 function update(kind,p,context){
  ...
  desert.visible=kind==='landing'&&state.altitudeKm<5;desert.scale.setScalar(.005);desert.position.set(0,.00001,0);desert.children[0].material.opacity=1;
  ...
  if(kind==='launch'){
   ...
   const payload=launch.group.localToWorld(launch.payloadPosition(p,state));state.cameraTarget=state.spacecraftReleased?payload.toArray():launch.group.position.toArray();state.displaySpanKm=profile.rocket.lengthKm;
   if(!state.spacecraftReleased){
   ...
   craft.group.visible=true;craft.group.scale.setScalar(craftScale);craft.group.position.copy(payload);craft.group.quaternion.copy(launch.group.quaternion);craft.setSolarDeployment(state.spacecraftReleased?Math.min(1,(state.elapsedSeconds-state.events.spacecraftSeparation)/40):0);
   if(state.spacecraftReleased)state.displaySpanKm=profile.rocket.lengthKm*.7;
   ...
   craft.group.visible=capsule.visible=true;craft.group.scale.setScalar(craftScale);craft.group.position.set(...state.spacecraft.positionKm);craft.group.quaternion.setFromUnitVectors(down,forward);craft.capsule.visible=false;
   if(id==='hayabusa2'){
   ...
   state.cameraTarget=craft.group.position.clone().add(capsule.position).multiplyScalar(.5).toArray();state.displaySpanKm=Math.max(profile.spacecraft.spanKm*1.7,craft.group.position.distanceTo(capsule.position)*1.15);
