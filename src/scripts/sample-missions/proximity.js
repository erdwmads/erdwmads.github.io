...
export function sampleTrack(track,p){
 ...
 return {position,velocity,time:new Date(Date.parse(a.time)+u*dt).toISOString()};
}
export function proximityShot(state,focus,aspect=1.9){
 const {position,extent,diameterKm,spanKm}=state;let target,offset,near;
 if(focus==='asteroid'){target=[0,0,0];offset=[1.5,.9,2].map(v=>v*diameterKm);near=diameterKm*.002;}
 ...
 const framing=Math.max(1,Math.min(1.9,1.25/aspect));return {target,position:target.map((v,i)=>v+offset[i]*framing),fov:40,near,far:Math.max(150,extent*8),minDistance:focus==='spacecraft'?spanKm*.7:focus==='asteroid'?diameterKm*.8:extent*.1,maxDistance:extent*8};
}
