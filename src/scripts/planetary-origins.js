...
export function createOriginsScene(material,light=false,volumeData) {
  ...
  function update(progress,cutaway) {
    ...
    cloud.visible=phase<2;body.visible=phase===1&&t>.38||phase>=2;debris.visible=phase===3&&(material!=='orgueil'||t<.72);
    for(let i=0;i<grains.count;i++) {
      const s=seeds[i],spin=s.a+(phase===0?t*.55:.55+t*.5),contraction=phase===0?0:smooth(t);
      helper.position.set(Math.cos(spin)*s.r,Math.sin(spin)*s.r*.42+s.y,Math.sin(spin)*s.r*.25);
      helper.position.lerp(endpoint.copy(s.end).multiplyScalar(1.35),contraction);
      for(let j=0;j<4;j++) {
      ...
      helper.rotation.set(s.spin+t*.3,s.a+t*.4,s.spin*.3);helper.scale.copy(s.shape).multiplyScalar(s.size*(1+contraction*1.4)*(1-.95*smooth((contraction-.8)/.2)));helper.updateMatrix();grains.setMatrixAt(i,helper.matrix);
    }
    grains.instanceMatrix.needsUpdate=true;dustGeometry.attributes.position.needsUpdate=true;dust.material.opacity=phase===0?.6:.6*(1-smooth(t));
    body.scale.setScalar(phase===1?1.35*smooth((t-.38)/.62):1.35);
    volume.update(phase,t,cutaway);
  ...
  update(.125,.65);
  return {group,update,setFx,inspectionTarget:()=>group.userData.phase===2&&group.userData.cutaway>0&&group.userData.progress>.56?volume.inspectionTarget():null};
}
