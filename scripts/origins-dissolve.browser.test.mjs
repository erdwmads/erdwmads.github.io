...
try{
 ...
 await page.evaluate(()=>{study.select(3,.7);study.setProgress(.7);});const early=await page.evaluate(()=>study.state.camera);await page.evaluate(()=>study.setProgress(.98));const late=await page.evaluate(()=>study.state.camera);assert.deepEqual(early,late,'Final chapter keeps a fixed camera while fragments reaccumulate');await page.locator('#zoom-in').click();const manual=await page.evaluate(()=>study.state.camera);assert.notDeepEqual(manual,late,'Zoom moves the camera');await page.evaluate(()=>study.setProgress(.8));assert.deepEqual(await page.evaluate(()=>study.state.camera),manual,'Manual camera input survives timeline scrubbing');await page.locator('#reset').click();assert.deepEqual(await page.evaluate(()=>study.state.camera),early,'Reset restores the fixed chapter framing');
