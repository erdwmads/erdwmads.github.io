...
### Known failures (2026-09-25)

The first full run of this suite (2026-09-25) passed 50 of 84 tests. The other 34 also fail on `be10de7`, the commit before that run, with the same errors, so they describe UI that has since changed rather than regressions. Update or retire them before relying on them:

| Cause | Tests |
|---|---|
| Origins used to be a view inside the planetary field guide; it now lives on `origins-study.html`, but these still look for `[data-view="origins"]` or `[data-origin-progress]` in `.planetary` | `origins-contact`, `origins-controls-state`, `origins-explanation`, `origins-inspection`, `planetary-origins`, `planetary-origins-drag`, `planetary-origins-lifecycle`, `planetary-origins-lighting`, `planetary-origins-refinement`, `interaction-stability` (Research routes) |
| The mission viewer changed after these were written: the Vehicle close-up control is not visible in some chapters, default camera modes differ, and chapter selectors now match two elements | `earth-flyby`, `flight-frames`, `physical-missions`, `sample-camera`, `mission-continuity`, `mission-geography`, `sample-missions`, `sample-missions-interaction`, `sample-missions-refinement`, `research-review` |
| The unlock form now has two buttons (password and passkey), so `[data-research-lock-form] button` is ambiguous | `mission-experience`, `mission-layout` |
| Photography presentation expectations | `photo-flight`, `photo-wall`, `fullscreen-paint` |
| Origins study expectations | `origins-dissolve`, `origins-reading` |
| Other | `planetary-explorer` (its framing check counts the stage's rounded corners as model pixels), `site-refinement` (looks for the removed `.page-outline`), `prism-controls`, `progress-polish`, `site-audit`, `observatory`, `research-passkey-virtual` |
