...
try{const page=await browser.newPage({viewport:{width:1440,height:900},reducedMotion:'reduce'});
 ...
 assert.equal(await nojs.locator('[data-email-compose]').getAttribute('href'),'mailto:rivieraliuyong@fuji.waseda.jp');
 ...
 console.log('Paper restoration, native email and printable CV verified; PDF: '+path.join(os.tmpdir(),'reviewed-cv.pdf'));
